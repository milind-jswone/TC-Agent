...
Production master should use the 189-column schema supplied in `master_tc_data.xlsx`.
The first column is generated by the agent:
...
- One uploaded TC file gets one generated ID such as `TC_001`.
- Multiple coil or batch rows from the same TC repeat the same ID.
- The next uploaded TC file receives the next ID, such as `TC_002`.
- If the SharePoint master currently has the older temporary 37-column schema, the agent rebuilds it into the supplied 189-column schema on the next successful run.
